class TabLink {
  constructor(tabElement){
    ...
    console.log('dataset', this.cards)

     // Map over the newly converted NodeList we just created in our if statement above. 
    //  Convert each this.cards element into a new instance of the TabCard class. Pass 
    // in a card object to the TabCard class. 
    this.cards = Array.from(this.cards)
    this.cards.map(e => {
      new TabCard(e)
    })
    // .map(obj => {
    //   const newCards = new TabCard
    // });
  ...
  selectTab(){
    ...
    console.log('All cards, NONE', cards)
    
    // Add a class of ".active-tab" to this.tabElement
    this.tabElement.classList.add('active-tab')
    // this.tabElement = this.tabElement.classList.add('active-tab');
    console.log('tabElement ACTIVE', this.tabElement)
...
*/
let toptabs = document.querySelectorAll('.tab');

toptabs.forEach((tab,index) => {
  const tabsList = new TabLink(tab);
  console.log('tabs', tabsList, index);
})